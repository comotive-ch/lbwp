...
class ManualOrder
{
  ...
  public function addProduct($productId, $quantity, $variation = array())
  {
    ...
    if ($product->get_stock_quantity() >= $quantity) {
      ...
      // Add line item
      $itemId = wc_add_order_item($this->orderId, array(
      ...
      wc_add_order_item_meta($itemId, '_qty', $quantity);
      wc_add_order_item_meta($itemId, '_tax_class', '');
      wc_add_order_item_meta($itemId, '_line_total', $price);
      wc_add_order_item_meta($itemId, '_line_subtotal', $price);

      // Only add a variation if there is one
      if (isset($variation['variation_id']) > 0) {
        wc_add_order_item_meta($itemId, '_variation_id', $variation['variation_id']);
        wc_add_order_item_meta($itemId, $variation['variation_attr_name'], $variation['variation_attr_name_value']);
      }

      // line item total values
      foreach (array('_line_tax', '_line_subtotal_tax') as $key) {
        wc_add_order_item_meta($itemId, $key, woocommerce_format_total(0) );
      }
