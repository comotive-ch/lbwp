			...
			for (var fieldname in fields) {
				if (params[fields[fieldname]].value != "" && params[fields[fieldname]].value != null) {
					...
					for (var j in values) {
						...
						if (values[j].indexOf("field:") >= 0) {
							if (LbwpFormEditor.Data.Items[values[j].slice(6, values[j].length)] != undefined) {
								value = value.replace(values[j], LbwpFormEditor.Data.Items[values[j].slice(6, values[j].length)].params[0].value);
							} else {
								value = value.replace(values[j], "<i>Dieses Feld ist nicht (mehr) verfügbar</i>");
							}
						}
					}
		...
		jQuery(".ref").unbind("click").click(function () {
			...
			if (jQuery(this).hasClass("currentRef")) {
				...
				ref.children("a").click(function (e) {
					e.preventDefault();
					var dest = elem.parent().find("input,textarea");
	...
	loadConditions: function (key) {
		...
		for (var i in params) {
			if (params[i].key == "condition_type") {
				html += '\
					<input class="andRadio" type="radio" value="AND" name="type" ' + (params[i].value == "AND" || params[i].value == null ? "checked" : "") + '/>\
					<label data-for="andRadio">' + LbwpFormEditor.Text.conditionAndSelection + '</label><br>\
					<input class="orRadio" type="radio" value="OR" name="type" ' + (params[i].value == "OR" ? "checked" : "") + '/>\
					<label data-for="orRadio">' + LbwpFormEditor.Text.conditionOrSelection + '</label>\
				';
			}
		}
		...
		// Register a class like click handler
		jQuery('label[data-for=andRadio]').on('click', function() {
			jQuery('.andRadio').trigger('click');
		});
		jQuery('label[data-for=orRadio]').on('click', function() {
			jQuery('.orRadio').trigger('click');
		});
