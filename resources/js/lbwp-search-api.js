...
var LbwpSearchApi = {
	...
	getResults : function()
	{
		...
		var data = {
			search : typeof(QueryString.q) == 'string' ? QueryString.q : QueryString.s,
			lang : lbwpGlobal.language,
			page : ++LbwpSearchApi.currentPage
		};

		// Get the actual result from API
		jQuery.post('/wp-json/lbwp/search/result', data, function(response) {
			...
			// Remove eventual no results message if there are .result-item
			if (jQuery('.result-item').length > 0) {
