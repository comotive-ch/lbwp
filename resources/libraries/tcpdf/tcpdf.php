...
class TCPDF {
	 ...
	 */
	public function Text($x, $y, $txt, $fstroke=false, $fclip=false, $ffill=true, $border=0, $ln=0, $align='', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M', $rtloff=false, $w=0) {
		$textrendermode = $this->textrendermode;
		$textstrokewidth = $this->textstrokewidth;
		$this->setTextRenderingMode($fstroke, $ffill, $fclip);
		$this->SetXY($x, $y, $rtloff);
		$this->Cell($w, 0, $txt, $border, $ln, $align, $fill, $link, $stretch, $ignore_min_height, $calign, $valign);
		...
		switch($dest) {
			...
			case 'FD': {
				...
				if ($this->diskcache) {
					...
					$f = fopen($name, 'wb');
					if (!$f) {
					  // Debug code, added 02.11.2014
					  mail('matthias.zobrist@blogwerk.com', 'TCPDF Error mail', 'Error data: ' . PHP_EOL . print_r(error_get_last(), true));
