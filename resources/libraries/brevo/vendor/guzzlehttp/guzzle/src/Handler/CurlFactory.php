...
namespace BrevoScoped\GuzzleHttp\Handler;

use BrevoScoped\GuzzleHttp\Exception\ConnectException;
use BrevoScoped\GuzzleHttp\Exception\RequestException;
use BrevoScoped\GuzzleHttp\Promise as P;
use BrevoScoped\GuzzleHttp\Promise\FulfilledPromise;
use BrevoScoped\GuzzleHttp\Promise\PromiseInterface;
use BrevoScoped\GuzzleHttp\Psr7\LazyOpenStream;
use BrevoScoped\GuzzleHttp\TransferStats;
use BrevoScoped\GuzzleHttp\Utils;
use BrevoScoped\Psr\Http\Message\RequestInterface;
use BrevoScoped\Psr\Http\Message\UriInterface;
/**
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx): PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \BrevoScoped\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri): string
    {
        ...
        $redactedUriString = \BrevoScoped\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf): void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \BrevoScoped\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \BrevoScoped\GuzzleHttp\Psr7\Utils::streamFor($sink);
