 ...
 */
namespace BrevoScoped\Symfony\Component\VarDumper\Caster;

use BrevoScoped\Symfony\Component\ErrorHandler\Exception\FlattenException;
use BrevoScoped\Symfony\Component\ErrorHandler\Exception\SilencedErrorContext;
use BrevoScoped\Symfony\Component\VarDumper\Cloner\Stub;
        ...
        if (isset($f['file'], $f['line'])) {
            ...
            if (isset(self::$framesCache[$cacheKey])) {
                ...
                if (is_file($f['file']) && 0 <= self::$srcContext) {
                    if (!empty($f['class']) && is_subclass_of($f['class'], 'BrevoScoped\Twig\Template')) {
                        $template = null;
                        if (isset($f['object'])) {
                            $template = $f['object'];
