...
class Frontend extends Base
{
  ...
  public function queryEvents($config)
  {
    ...
    $config = apply_filters('lbwpEvents_before_query_events', $config);
    ...
    if (
      ...
      foreach ($config['taxonomies'] as $taxonomy) {
        if (is_array($config['terms'][$taxonomy]) && count($config['terms'][$taxonomy]) > 0) {
          $query['tax_query'][] = array(
            'taxonomy' => $taxonomy,
            'field' => 'slug',
    ...
    // Fire up the sql
    $query = apply_filters('lbwpEvents_before_query_events_query_object', $query);
  ...
  public function populateEventData(&$events, $config, $currentTime)
  {
    ...
    foreach ($events as $id => $event) {
      ...
      // For past check, assume end of day if no start date was given, so events stay on the page on the day they are starting
      $eventStartPastCheck = date('Hi', $eventStart) == '0000' ? $eventStart + 86400 : $eventStart;
      $eventEnd = intval(get_post_meta($event->ID, 'event-end', true));
      // If no endtime was set, assume it ends at the end of the day
      $eventEnd = (date('Hi', $eventEnd) == '0000' && $eventEnd > 0) ? $eventEnd + 86400 : $eventEnd;

      // Remove a past event, if past events should be removed
      if ($removePastEvents && $eventEnd > 0 && $eventEnd <= $currentTime) {
        unset($events[$id]);
        continue;
      } else if ($removePastEvents && $eventEnd == 0 && $eventStartPastCheck <= $currentTime) {
        unset($events[$id]);
        continue;
      }
      ...
      $event->mapLabel = get_post_meta($event->ID, 'event-map-label', true);
      $event->mapLinkText = get_post_meta($event->ID, 'event-map-link-text', true);
      $event->additionalLinks = array();
      $event->hideMapUrl = get_post_meta($event->ID, 'hide-map-url', true) == 'on';
      $event->hideDate = get_post_meta($event->ID, 'hide-date', true) == 'on';
      ...
      // Fallbacks if no map label/text is set
      if (strlen($event->mapLabel) == 0) {
        $event->mapLabel = __('Karte', 'lbwp');
      }
      if (strlen($event->mapLinkText) == 0) {
        $event->mapLinkText = __('Ort auf der Karte anzeigen', 'lbwp');
      }
      // Add additional links, if given
      foreach (array(1,2,3) as $index) {
        $url = get_post_meta($event->ID, 'event-link-' . $index, true);
        if (strlen($url) > 0) {
          $event->additionalLinks[] = array(
            'url' => $url,
            'label' => get_post_meta($event->ID, 'event-link-label-' . $index, true),
            'text' => get_post_meta($event->ID, 'event-link-text-' . $index, true)
          );
        }
      }
  ...
  public function getDataListHtml($event, $textdomain = 'lbwp', $display = array(), $override = array())
  {
    ...
    if ($event->startTime > 0 && $event->endTime > 0 && $display['showDates'] && !$event->hideDate) {
        ...
        if ($hasStartTime && $hasEndTime) {
          ...
          $html .= '
            <dl>
              ...
              <dd>' . sprintf(apply_filters('lbwpEvents_single_datalist_start_end', __('Von %s bis %s', 'lbwp')), $startTime, $endTime) . '</dd>
            </dl>
          ';
    ...
    } else if ($event->startTime > 0 && $event->endTime == 0 && $display['showDates'] && !$event->hideDate) {
    ...
    if ($display['showLocation']) {
      ...
      if ($display['showAddressAfterLocation'] && strlen(trim(strip_tags($address))) > 0) {
        $html .= '
          <dl>
            <dt>' . __('Ort', 'lbwp') . '</dt>
    ...
    if ($display['showMapLinks'] && !$event->hideMapUrl && strlen($event->mapUrl) > 0) {
      $html .= '
        <dl>
          <dt>' . $event->mapLabel . '</dt>
          <dd><a href="' . $event->mapUrl . '" target="_blank">' . $event->mapLinkText . '</a></dd>
        </dl>
      ';
      // Also print additional links if given
      foreach ($event->additionalLinks as $link) {
        $html .= '
          <dl>
            <dt>' . $link['label'] . '</dt>
            <dd><a href="' . $link['url'] . '" target="_blank">' . $link['text'] . '</a></dd>
          </dl>
        ';
      }
    }
    ...
    // If basically empty, flush the wrapper so its not displayed empty and reserving space
    if (strlen($html) === 35) {
      $html = '';
    }
  ...
  public static function getCombinedLocationAndAddress($event)
  {
    ...
    return '<span>' . implode(',</span> <span>', array_filter($parts)) . '</span>';
  }
  ...
  public function downloadCalendarFile()
  {
    ...
    if ($event->ID > 0 && isset($event->startTime) && $event->startTime > 0) {
      ...
      echo self::getCalendarFileString($event);
      exit;
    }
  }
   ...
   * @return string
   */
  public static function getCalendarFileString($event)
  {
    $str = '';
    // Print the calendar minimal output
    $str .= 'BEGIN:VCALENDAR' . PHP_EOL;
    $str .=  'VERSION:2.0' . PHP_EOL;
    $str .=  'PRODID:' . get_bloginfo('url') . PHP_EOL;
    $str .=  'METHOD:PUBLISH' . PHP_EOL;
    $str .=  'BEGIN:VEVENT' . PHP_EOL;
    // TODO This actually sends a invitation which we don't want until it's configurable
    //$str .=  'UID:' . $email . PHP_EOL;
    //$str .=  'ORGANIZER;CN="' . get_bloginfo('name') . '":MAILTO:' . $email . PHP_EOL;
    $str .=  'LOCATION:' . self::getCombinedLocationAndAddress($event) . PHP_EOL;
    $str .=  'SUMMARY:' . trim(preg_replace('/\s\s+/', ' ', $event->post_title)) . PHP_EOL;
    // USe excerpt if given or post content if not
    if (strlen($event->post_excerpt) > 0) {
      $str .=  'DESCRIPTION:' . strip_tags(trim(preg_replace('/\s\s+/', ' ', $event->post_excerpt))) . PHP_EOL;
    } else {
      $str .=  'DESCRIPTION:' . strip_tags(trim(preg_replace('/\s\s+/', ' ', $event->post_content))) . PHP_EOL;
    }
    $str .=  'CLASS:PUBLIC' . PHP_EOL;
    $str .=  'DTSTART:' . date(Date::ICS_DATE, $event->startTime) . PHP_EOL;
    if (isset($event->endTime) && $event->endTime > 0 && $event->endTime != $event->startTime) {
      $str .=  'DTEND:' . date(Date::ICS_DATE, $event->endTime) . PHP_EOL;
    }
  ...
  public function getEventSubscriptionHtml($event)
  {
    ...
    add_filter('lbwpForms_load_form_shortcode', function ($shortcode, $form) use ($event) {
      ...
      // See if theres more than one email
      $emails = explode(';', $event->subscribeEmail);
      // Check if there is at least one valid subscribe email and the form is actually from the event
      if (!Strings::checkEmail($emails[0]) || $form->ID != $event->subscribeFormId) {
        return $shortcode;
      }
      ...
      $elements[] = $formHandler->generateActionItem(array(
        'key' => 'sendmail',
        // This can be multiple with semicolon
