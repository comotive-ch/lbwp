...
abstract class Base
{
  ...
  public function getFieldContent($fields, $value, $exactMatch = false)
  {
    ...
    if (Strings::contains($value, 'field:')) {
      foreach ($fields as $field) {
        if (!$exactMatch) {
          // Blindly replace every replaceable field by name and id, name only if given
  ...
  public function isExecuteable($data)
  {
    ...
    if (isset($this->params['conditions']) && strlen($this->params['conditions']) > 0 && strlen($this->params['condition_type']) > 0) {
      ...
      foreach ($conditions as $condition) {
        ...
        foreach ($data as $field) {
          if ($field['id'] == $fieldName && $checkValue == $field['value']) {
            ++$conditionsMet;
          }
        }
      }
