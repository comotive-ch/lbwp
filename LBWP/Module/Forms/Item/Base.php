...
abstract class Base
{
  ...
  protected $params = array(
    ...
    'condition_type' => 'and',    // Condition type default AND
  ...
  protected $paramConfig = array(
    ...
    */
    ...
    'condition_type' => array(
      'name' => 'Verhalten von Konditionen',
      'type' => 'radio',
      'help' => 'Wenn das Feld mittels mehrere Konditionen gesteuert wird, entscheidet die Auswahl, ob alle oder eine Kondition zutreffen müssen',
      'values' => array(
        'and' => 'Alle müssen zutreffen',
        'or' => 'Eine muss zutreffen'
      )
    ),
    'conditions' => array(
  ...
  public function __construct($core)
  {
    ...
    $this->defaultWarningMessage = __('Bitte überprüfe den Feld-Inhalt.', 'lbwp');
    $this->defaultErrorMessage = __('Bitte fülle das Feld "%s" aus.', 'lbwp');
  }
  ...
  protected function getDefaultAttributes(&$args, $class = '', $styles = '', $requiredAttributes = true)
  {
    ...
    // Set calculation configuration if given
    if (isset($args['automatic_calculation']) && $args['automatic_calculation'] == 'ja') {
      $attr .= ' data-auto-calc-on="1"';
      $attr .= ' data-auto-calc-syntax="' . esc_attr($args['calculation_syntax']) . '"';
    }
  ...
  {
    $type = isset($this->params['condition_type']) && strlen($this->params['condition_type']) > 0 ? $this->params['condition_type'] : 'and';
    $conditions = $this->decodeObjectString($conditions);
    foreach ($conditions as $key => $condition) {
      // We don't need truncate actions anymore as they are auto-triggered with hide actions
      if ($condition['action'] == 'truncate') {
        unset($conditions[$key]);
        continue;
      }
      $conditions[$key]['target'] = $this->params['id'];
      $conditions[$key]['type'] = $type;
      $conditions[$key]['reverse'] = $this->getReverseCondition($condition)['action'];
    }
