...
class Dropdown extends Base
{
  ...
  public function getValue($args = array())
  {
    ...
    if (isset($_REQUEST[$this->get('id')]) && strlen($_REQUEST[$this->get('id')]) > 0) {
      $value = $_REQUEST[$this->get('id')];
    ...
    // Allow the use of vorgabewert (which can only be done programatically)
    if (strlen($this->get('vorgabewert')) > 0) {
      return $this->get('vorgabewert');
    }
