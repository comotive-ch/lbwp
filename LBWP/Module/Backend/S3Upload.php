...
class S3Upload extends \LBWP\Module\Base
{
  ...
  public function deleteFile($filename)
  {
	...
	/**
	 * Delete a file by the key
	 * @param  string $filename The key of the file e.g. wp_test/files/1631704627/file.jpg
	 */
	public function deleteFileByKey($filename)
  {
    $s3 = AwsFactoryV3::getS3Service();
    // Delete it.
    if (!defined('LOCAL_DEVELOPMENT')) {
  ...
  /**
   * @param string $file native disk file path
   ...
   * @param string $remoteName the path to save to
   * @param string $type the mime type of the file
   * @param bool $skipMaxImageSize skip max image size filter
   * @return string the final url of the file
   */
  public function uploadDiskFileFixedPath($file, $remoteName = '', $type = '', $skipMaxImageSize = false)
  {
    // Move the file to a local temp path
    $localFile = $this->fixAndRename($file);
    // Create the url
    $s3Url = $this->changeUploadUrl('') . $remoteName;
    // Do the upload
    if ($this->handleUpload($localFile, $s3Url, $type, $skipMaxImageSize)) {
      return $s3Url;
    }
  ...
  public function handleUpload($localFile, $url, $mime_type = '', $skipMaxImageSize = false)
  {
    ...
    // Always skip max image check for gifs
    if (File::getExtension($localFile) == '.gif') {
      $skipMaxImageSize = true;
    }
    ...
    if ($result != null && strlen($result->get('ObjectURL')) > 0) {
      return true;
    } else {
      return false;
    }
   ...
   * Handles the upload to CDN
   * @param string $localFile The local file name
   * @param string $key a final key where the file should be uploaded
   * @param string $mime_type The mime type of the file
   * @return bool true/false if the upload is successful
   */
  public function handleKeyUpload($localFile, $key, $mime_type = '')
  {
    $s3 = AwsFactoryV3::getS3Service();
    // Just for the backup-image sake, we need to handle the error that a local file does not exist as a non-error
    if (!file_exists($localFile)) {
  ...
  public function setAccessControl($key, $acl)
  {
    ...
    try {
      ...
      SystemLog::add('S3::setAccessControl', 'critical', 'putObjectAcl Exception', array(
        'Key' => $key,
        'Acl' => $acl
      ));
    }
   ...
   */
  public function handleMaxImageSize($localFile)
  {
  ...
  public function getFileLastModified($fileName){
    // Get the client
    AwsFactoryV3::setKeys(CDN_ACCESS_KEY, CDN_SECRET_KEY);
    $s3 = AwsFactoryV3::getS3Service();
    ...
    try {
      ...
      // Return the file last modified timestam if available
