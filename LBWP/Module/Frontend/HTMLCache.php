...
class HTMLCache extends \LBWP\Module\Base
{
  ...
  {
		parent::__construct();
		// Eventually use original request uri
    self::$uri = (isset($_SERVER['REQUEST_URI_ORIGINAL']))
      ? $_SERVER['REQUEST_URI_ORIGINAL']
      : $_SERVER['REQUEST_URI'];
	}
  ...
  public function initialize()
  {
    if (!is_admin() || isset($_GET['forceHtmlCache'])) {
      ...
      $currentPaths = parse_url(self::$uri);
  ...
  /**
   * @param string $group override the cache group when a site with same url needs to be cached differently for some reason
   */
  public function setCurrentCacheGroup($group)
  {
    $this->currentCacheGroup = $group;
  }
  ...
  public static function cleanPostHtmlCache($postId)
  {
    ...
    $uri = $htmlCache->getUriPath(get_permalink($postId));
    $htmlCache->clearHtmlCache($uri);
  }
   ...
   * @return void
   */
  public static function cleanPageCacheByUri($uri)
  {
    $htmlCache = LbwpCore::getModule('HTMLCache');
    if (!($htmlCache instanceof HTMLCache)) {
      $htmlCache = new HTMLCache();
   ...
   * @param string $uri
   */
  public function clearHtmlCache($uri)
  {
    self::invalidatePage($uri);
  }
    ...
    if ($this->isCachable()) {
      ...
      foreach (headers_list() as $header) {
        if (
          ...
          Strings::startsWith(strtolower($header), 'cache-control') ||
          Strings::startsWith(strtolower($header), 'redirect') ||
          Strings::startsWith(strtolower($header), 'content-security-policy') ||
  ...
  protected function isCachable()
  {
    ...
    // Don't cache woocommerce password forgot process and add to cart
    if (isset($_GET['show-reset-form']) || (isset($_GET['key']) && isset($_GET['id']) && $_GET['add-to-cart'])) {
      return false;
    }
    // Every post request can't be cached (made for woocommerce)
    if ($_SERVER['REQUEST_METHOD'] === 'POST' || isset($_GET['ApiSignature'])) {
      return false;
    }
  ...
  /**
   * Allows caching for the current call
   */
   ...
   * @return void
   */
  public static function cacheThisPage($duration)
  {
    $component = new HTMLCache();
    $component->initialize();
    $component->setCacheTimes($duration, $duration);
  }
