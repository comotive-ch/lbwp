...
use const Raptus\WC\BexioConnector\PLUGIN_DIR;
...
class Multilang
{
  ...
  public static function isActive()
  {
    if (self::$active == NULL) {
      self::$active = (
        WordPress::isPluginActive('polylang-pro/polylang.php') ||
        WordPress::isPluginActive('polylang/polylang.php')
      );
    }
    // Override active=true, when in the course of disabling polylang
    if (isset($_GET['plugin']) && $_GET['action'] == 'deactivate' && str_starts_with($_GET['plugin'], 'polylang')) {
      self::$active = false;
    }
   ...
   * @return bool
   */
  public static function isPostTypeTranslated($type)
  {
    if (self::isActive()) {
      return pll_is_translated_post_type($type);
    }
    return false;
  }
  ...
  {
    if (self::isActive()) {
      foreach (pll_languages_list(array('fields' => false)) as $language) {
   ...
   * @param array $sidebar the sidebar object
   */
   ...
   */
  public static function addPostTranslation($originalPostId, $newPostId, $language)
  {
    if (Multilang::isActive()) {
   ...
   * @return bool
   */
  public static function isWeGlot()
  {
    return WordPress::isPluginActive('weglot/weglot.php');
  }

  public static function resetTextDomainOnRest($name, $type, $language)
  {
    if (defined('REST_REQUEST')) {
      PLL()->curlang = PLL()->model->get_language(sanitize_key($language));
      // Also, reload the text domain
      unload_textdomain($name);
      $locale = $language . '_' . strtoupper($language);
      switch_to_locale($locale);
      remove_all_filters('load_textdomain_mofile');
      switch ($type) {
        case 'lbwp':
          load_textdomain($name, WP_PLUGIN_DIR . '/lbwp/resources/languages/lbwp-' . $locale . '.mo', $locale);
          break;
        case 'plugin':
          load_plugin_textdomain($name);
          break;
        case 'theme':
          load_textdomain($name, get_stylesheet_directory() . '/assets/languages/' . $locale . '.mo', $locale);
          load_textdomain($name, get_template_directory() . '/assets/languages/' . $locale . '.mo', $locale);
          break;
      }
   ...
   */
  public static function getWeGlotLanguage()
  {
    if (self::isWeGlot()) {
   ...
   * @return array
   */
  public static function getWeGlotLanguages()
  {
    $languages = array();
    /** @var \Weglot\Client\Api\LanguageEntry $language */
    foreach (weglot_get_languages_available() as $language) {
      $languages[$language->getInternalCode()] = $language->getLocalName();
    }
  ...
  /**
   * Get a language on any (polylang or weglot) active language plugin
   * @return false|string false on failure or the language string (slug)
   */
  public static function getAnyPluginLang(){
    if(self::isWeGlot()){
      return self::getWeGlotLanguage();
