...
class LbwpData
{
   ...
   * @param int $limit a limit to retrieve
   * @return array of all current rows in keyspace
   */
  public function getRows($orderBy = 'pid', $order = 'ASC', $limit = 0, $page = 0)
  {
    $sql = '
  ...
  public function countRows($search = ''){
    $sql = 'SELECT COUNT(pid) FROM {raw:lbwpData} WHERE row_key = {rowKey}';
   ...
   * @return array
   */
  public function getRowsByDate($from, $to, $field = 'row_created')
  {
    $sql = 'SELECT * FROM {raw:lbwpData} WHERE row_key = {rowKey} AND {raw:field} BETWEEN {from} AND {to}';
    // Get the native rows
    $data = $this->db->get_results(Strings::prepareSql($sql, array(
      'lbwpData' => $this->db->lbwp_data,
      'rowKey' => $this->key,
  ...
  /**
   * @param int $pid
   * @param string $modified sql date
   * @return void
   */
  public function setRowModifiedByPid($pid, $modified)
  {
    $this->db->update(
      $this->db->lbwp_data,
      array(
   ...
   */
  public function updateRow($rowId, $data, $modified = false)
  {
    ...
    if ($this->rowExists($rowId)) {
      return $this->db->update(
        ...
        array(
          'user_id' => $this->userId,
          'row_modified' => (!$modified) ? current_time('mysql') : $modified,
  ...
  public function deleteOldRows($days = 90){
    $sql = 'DELETE FROM {raw:lbwpData} WHERE row_key = {rowKey} AND row_created < DATE_SUB(NOW(), INTERVAL {days} DAY)';
    $deleted = $this->db->query(Strings::prepareSql($sql, array(
      'lbwpData' => $this->db->lbwp_data,
      'rowKey' => $this->key,
      'days' => $days
    )));
