...
abstract class FetchComponent extends Component
{
    ...
    foreach ($this->urls as $section => $urls) {
      ...
      foreach ($languages as $language) {
        ...
        if (isset($urls[$language])) {
          ...
          $sectionContent = Fetch::getContent($urls[$language], $this->ignore404, $this->useProxy);
            ...
            foreach ($this->fetchFilters[$section] as $priority) {
              ...
              foreach ($priority as $callback) {
                ...
                if ($newSectionContent) {
                  ...
                  SystemLog::add('Fetch', 'critical', 'Fetch.' . $callback[1] . ' returned an error in section ' . $section . '.' . $language);
                  $abort = true;
  ...
  /**
   * Optional function to flush frontend cache, after execute
   */
