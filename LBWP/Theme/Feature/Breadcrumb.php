...
class Breadcrumb
{
  ...
  public static function init($options = array())
  {
    ...
    $defaults = array(
      ...
      'show' => array(
        ...
        'all_element_after_root_term' => false
      ),
      ...
      'display_position_property' => false,
      'use_post_type_archive_link' => false,
      'hide_singular_breadcrumb_by_type' => array(),
  ...
  public function fill($elements = array())
  {
    ...
    if (!is_front_page()) {
      ...
      if (is_home()) {
        ...
        // Add element for tax
        $this->fillTax($queried_object->taxonomy, $queried_object->term_id, true);
      } else if (is_date()) {
        // Display a date archive (any)
        $this->fillDate(get_query_var('year'), get_query_var('monthnum'), get_query_var('day'));
      } else if (is_search()) {
        // Search results are displayed
        $this->addElement('search', htmlentities(strip_tags(get_query_var('s'))), get_bloginfo('url') . '?s=' . urlencode(get_query_var('s')));
      } else if (is_author()) {
        $this->fillPagesToBlog();
        $this->addElement(
          'author',
        ...
        if (in_array($post->post_type, $this->options['hide_singular_breadcrumb_by_type'])) {
          $this->options['link_last_element'] = true;
   ...
   * @return void
   */
  protected function removeLastElement()
  {
   ...
   * @param int $term_id
   */
  protected function fillTax($taxonomy, $term_id, $istaxquery = false)
  {
    ...
    $this->addElement($taxonomy, $term->name, get_term_link($term, $taxonomy), $term_id);
    // If activated, show an "all" term, suggesting all subterms are shown, when root category
    if ($istaxquery && $this->options['show']['all_element_after_root_term'] && $term->parent == 0) {
      $this->addElement($taxonomy . '_all', __('Alle', 'lbwp'), '', $term_id);
    }
  }
            ...
            if ($terms != false) {
              ...
              foreach ($terms as $term) {
                if (!in_array($term->slug, $this->options['skip_categories'])) {
                  $deepestTerm = $term;
                  // Use any term with a parent, first found
                  if ($deepestTerm->parent > 0) {
                    break;
                  }
                }
              }
              if ($deepestTerm instanceof \WP_Term) {
                $this->fillTax($this->options['show']['taxonomy_before_single'], $deepestTerm->term_id);
              }
            }
            break;
        }
   ...
   * @param string $id a specific slug or id to be used as data attribute
   */
  public function addElement($type, $title, $link, $id = '')
  {
  ...
  public function display()
  {
    ...
    $this->elements = apply_filters('lbwp_before_display_breadcrumb_elements', $this->elements, $this->options);

    if (isset($this->options['classes']['without-default-class']) && $this->options['classes']['without-default-class'] === true) {
      $defaultClass = '';
    }

    echo '<ul class="' . $defaultClass . ' ' . $this->options['classes']['breadcrumb_class'] . '" itemscope itemtype="https://schema.org/BreadcrumbList" typeof="BreadcrumbList" name="Navigation">';
  ...
  {
    $class = ' class="' . $this->options['classes']['entry_class'];
    $class.= ' element_' . $element['type'];
    $class.= ' element-pos-' . $position;
    if ($last == true) {
      $class .= ' ' . $this->options['classes']['current_class'];
    }
    $class .= '"';

    echo '<li' . $class . ' itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem" data-id="' . $element['id'] . '">';

    $this->displayPrepostfix('prefix', $element['type']);

    if ($element['link'] != '') {
      if ($this->options['classes_on_links'] == false) {
        echo '<a href="' . $element['link'] . '" itemprop="item">';
      } else {
        echo '<a' . $class . ' href="' . $element['link'] . '" itemprop="item">';
      }
    }

    if ($this->options['display_name_property']) echo '<span itemprop="name">';
    ...
    if ($this->options['display_position_property']) {
  ...
  public function displayTitle($title)
  {
    ...
    // if there is html in the title, skip too
