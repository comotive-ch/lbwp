...
class Buttons
{
  ...
  protected $config = array(
    ...
    'buttons' => array(
      ...
      SocialApis::WHATSAPP => array(
        'class' => '\LBWP\Theme\Feature\SocialShare\Button\Whatsapp'
      ),
  ...
  {
    echo self::$instance->getHtml($args);
  }

  /**
   * returns the full button list html
   */
  public static function get($args = array())
  {
    return self::$instance->getHtml($args);
  }
  ...
  public function load($config)
  {
    ...
    // Remove google plus if still there
    if (isset($this->config['buttons'][SocialApis::GOOGLE_PLUS])) {
      $key = array_search(SocialApis::GOOGLE_PLUS, $this->config['order']);
      unset($this->config['buttons'][SocialApis::GOOGLE_PLUS]);
      unset($this->config['order'][$key]);
    }
    ...
    // Register a shortcode to add the buttons whenever wherever
    add_shortcode('lbwp:sharebuttons', array($this, 'printAsShortcode'));
  }
   ...
   * @return string
   */
  public function printAsShortcode($args)
  {
    return $this->getHtml();
  }
  ...
  {
    if (!is_singular($this->config['auto_insert'])) {
      return $content;
    }
  ...
  public function getHtml($args = array())
  {
    ...
    foreach ($this->config['order'] as $key) {
      ...
      // Add url param config to the actual link
      if (isset($config['urlParams'])) {
        if (Strings::contains($link, '?')) {
          $link .= '&' . $config['urlParams'];
        } else {
          $link .= '?' . $config['urlParams'];
        }
      }
