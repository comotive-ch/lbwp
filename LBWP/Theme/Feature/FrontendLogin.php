...
class FrontendLogin
{
  ...
  public function processLogin()
  {
    ...
    if ('POST' == $_SERVER['REQUEST_METHOD'] && $_POST['sentForm'] == 'login') {
  ...
  public function processRegistration()
  {
    ...
    if ('POST' == $_SERVER['REQUEST_METHOD'] && $_POST['sentForm'] == 'register') {
      $errorMessage = '';
      // Let developers do custom validation and return an error
      $userId = apply_filters('FrontendRegistration_custom_validation', false);
      // Let developers also do custom validation and return an actual message
      $errorMessage = apply_filters('FrontendRegistration_custom_validation_message', $errorMessage);
      // Check the passwords that are used for registration (only if there isn't already a message
      if (strlen($errorMessage) == 0) {
        $errorMessage = Strings::checkPasswords($_POST['password'], $_POST['password-confirm'], 8, 'lbwp');
      }

      // Only proceed if custom validation threw no errors
      if ($userId === false && strlen($errorMessage) == 0) {
      ...
      if (!is_wp_error($userId) && strlen($errorMessage) == 0) {
        if (isset($_POST['redirect_to']) && Strings::isURL($_POST['redirect_to'])) {
          wp_safe_redirect($_POST['redirect_to']);
        } else {
          $url = get_permalink();
    ...
    if (is_wp_error($userId)) {
      ...
      $html .= '<p class="message message-error lbwp-form-message error">' . implode('<br />', $errors) . '</p>';
    }

    if (strlen($errorMessage) > 0) {
      $html .= '<p class="message message-error lbwp-form-message error">' . $errorMessage . '</p>';
    }
  ...
  public function displayRegistrationForm()
  {
    ...
    $shortcode = '
      [lbwp:form button="' . __('Registrieren', 'lbwp') . '" id="register" weiterleitung="' . get_permalink() . '" action="' . get_permalink() . '" skip_execution="1"]
        ...
        [lbwp:formItem key="textfield" pflichtfeld="ja" id="password-confirm" feldname="' . __('Passwort bestätigen', 'lbwp') . '" type="password"]
        ' . apply_filters('FrontendRegistration_shortcode_content_end', '') . '
    ...
    if (isset($_POST['sentForm'])) {
      ...
      if ($_POST['sentForm'] == 'email-reset') {
        ...
        if ($user instanceof \WP_User) {
          ...
          ';
          $email = apply_filters('FrontendReset_filter_email_object', $email, $url, $user);
          $email->send();
