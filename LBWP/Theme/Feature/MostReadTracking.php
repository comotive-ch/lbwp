...
class MostReadTracking
{
  ...
  protected function load()
  {
    ...
    if (!is_user_logged_in() || defined('LOCAL_DEVELOPMENT')) {
      ...
      add_action('wp_ajax_most_read_track_hit', array($this, 'trackHitAjax'));
    }
  ...
  public function trackHitAjax()
  {
    ...
    WordPress::sendJsonResponse(array('c' => $this->trackHit($postId)));
  }
  ...
  {
    if ($postId > 0 && !Strings::isSearchEngineUserAgent() && !Strings::isScraperUserAgent()) {
   ...
   * @return array
   */
  public static function getPosts($number, $type = 'post', $fillRandom = false, $mergeArgs = array())
  {
    $args = self::getQueryParams();
    $args['post_type'] = $type;
    $args['posts_per_page'] = $number;
    // Merge args if given
    if (count($mergeArgs) > 0) {
      $args = array_merge($args, $mergeArgs);
    }
    ...
    if ($fillRandom && $number < count($posts)) {
      // not enough posts? get random posts to fill
      $postIds = array();
