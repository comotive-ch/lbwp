...
class PrintToPdf
{
  ...
  public function onSaveFlushPdfData($postId)
  {
    ...
    $attachmentId = intval(get_post_meta($postId, self::META_PDF_ATTACHMENT_ID, true));
  ...
  protected function generatePostPdf($postId)
  {
    ...
    // Create a local attachment if needed
    if ($this->options['useAttachments']) {
      ...
      update_post_meta($attachmentId, '_wp_attached_file', str_replace(LbwpCore::getCdnFileUri(), '', $fileUrl));
