...
class AutoUnpublishPost
{
   ...
   * @return void
   */
  public function eventuallyRedirect()
  {
    if (is_singular() && !isset($_GET['preview'])) {
      $postId = get_the_ID();
      $url = get_post_meta($postId, 'auto_unpublish_redirect_url', true);
      // Get the unpublish date
      $timestamp = intval(get_post_meta($postId, 'auto_unpublish_on', true));
      if (strlen($url) > 0 && $timestamp < current_time('timestamp')) {
        $_SESSION['lbwp_auto_unpublish_message'] = get_post_meta($postId, 'auto_unpublish_message', true);;
        wp_redirect($url);
        exit;
      }
    }
  ...
  public function displayMetabox($post)
  {
    ...
    if ($this->settings['allowRedirects']) {
      $url = get_post_meta($post->ID, 'auto_unpublish_redirect_url', true);
      $message = get_post_meta($post->ID, 'auto_unpublish_message', true);
      $html .= '
        <p>
          <label>
            ' . __('Weiterleitung einrichten (URL)', 'lbwp') . '
            <input type="text" name="auto_unpublish_redirect_url" value="' . esc_attr($url) . '" />
          </label>
        </p>
        <p>
          <label>
            ' . __('Eigene Fehlermeldung', 'lbwp') . '
            <input type="text" name="auto_unpublish_message" value="' . esc_attr($message) . '" />
          </label>
        </p>
      ';
    }
  ...
  public function savePostUnpublishHook($postId)
  {
    ...
    // Save the redirect url and message
    if ($this->settings['allowRedirects']) {
      update_post_meta($postId, 'auto_unpublish_redirect_url', $_POST['auto_unpublish_redirect_url']);
      update_post_meta($postId, 'auto_unpublish_message', $_POST['auto_unpublish_message']);
    }
  }
  ...
  {
    $sql = 'SELECT post_id FROM {sql:postMeta} WHERE meta_key = {metaKey} AND meta_value <= {now} AND meta_value != ""';
    ...
    foreach ($postIds as $postId) {
      // Sort out those having a redirect, we can't draft these
      if ($this->settings['allowRedirects']) {
        $url = get_post_meta($postId, 'auto_unpublish_redirect_url', true);
        if (strlen($url) > 0) {
          continue;
