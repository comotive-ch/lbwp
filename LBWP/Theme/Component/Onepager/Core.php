...
abstract class Core extends BaseComponent
{
  ...
  public function init()
  {
    ...
    $this->templates = apply_filters('OnePagerTemplate_filter', $this->templates);
  ...
  public function duplicateItems($oldPostId, $newPostId, $type)
  {
    ...
    if (in_array($type, $this->types) && get_post_meta($newPostId, 'onepager-active', true) == 'on') {
      ...
      $newElementIds = array();
      $elements = array_filter(get_post_meta($oldPostId, 'elements', false));
      foreach ($elements as $itemId) {
        ...
        foreach ($fmeta as $k => $v) {
          if (is_array($v)) {
            foreach ($v as $iv) {
              add_post_meta($newItemId, $k, $iv);
            }
          } else {
            update_post_meta($newItemId, $k, maybe_unserialize($v));
          }
        }
      }
  ...
  {
    // Don't save on bulk edit, quick edit and trashing
    if (
  ...
  public function getItems($id)
  {
    ...
    if (count($this->instances) == 0) {
      ...
      foreach ($elements as $postId) {
        ...
        if (
          $postId > 0 &&
          ($postObject->post_status == 'publish' || current_user_can('edit_posts')) &&
          apply_filters('OnePager_filter_element_visibility', true, $postObject)
        ) {
          $type = get_post_meta($postId, 'item-type', true);
          /** @var BaseItem $element */
          $class = $this->items[$type]['class'];
