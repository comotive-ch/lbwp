...
class Implementation extends Base implements Definition
{
   ...
   * @var bool
   */
  protected $directLoad = false;

  /**
   * @param \LBWP\Newsletter\Core $core
   */
  public function __construct($core, $directLoad = false)
  {
  ...
  {
    if ($this->isWorking() && ($this->directLoad || stristr($this->core->getSettings()->get('serviceClass'), 'LocalMail') !== false)) {
  ...
  public function displaySettings()
  {
    ...
    if ($this->isWorking()) {
      ...
      $fields = '';
      $currentListId = isset($this->settings['listId']) ? intval($this->settings['listId']) : '';
      $input = '<select name="listId">' . $this->getListOptions($currentListId, 'listId', true) . '</select>';
      ...
      // Create input and description
      $currentTestId = isset($this->settings['testId']) ? intval($this->settings['testId']) : '';
      $input = '<select name="testId">' . $this->getListOptions($currentTestId, 'testId', true) . '</select>';
  ...
  public function getListOptions($selectedKeys = array(), $fieldKey = 'listId', $includeNoSelection = false)
  {
    ...
    $lists = apply_filters('lbwp_localmail_list_options', $this->api->getLists());
    $selectedKeys = array_filter(ArrayManipulation::forceArrayAndInclude($selectedKeys));
      ...
      foreach ($lists as $id => $list) {
        ...
        foreach ($selectedKeys as $selectedKey) {
          if (strlen($selectedKey) > 0) {
            $selected = selected($selectedKey, $id, false);
          }
  ...
  public function createMailing($targets, $html, $text, $subject, $senderEmail, $senderName, $originalTarget, $language, $newsletter)
  {
    ...
    $reduceSegment = array();
    if ($newsletter->useReduceSegment() == 1) {
      $reduceSegments = true;
      $reduceSegment = ArrayManipulation::forceArray($newsletter->getReduceSegment());
    }
    ...
    foreach ($targets as $listId) {
      ...
      if (is_array($list) && count($list)) {
        foreach ($list as $memberId => $recipient) {
          // Skip not part of segment reduction
          if ($reduceSegments && !in_array($memberId, $reduceSegment)) {
            continue;
          }
          ...
          // Might be used if personalized preview links are made
          $recipient['preview-id'] = $memberId . '-' . $emailListId;

          // Personalize the mailing text with user data
          $personalizedHtml = $html;
          $useSalutation = stristr($personalizedHtml, '{salutation}') !== false;
          $recipient = apply_filters('lbwp_nl_user_data_before_add', $recipient, $useSalutation);
          ...
          // Replace inline onclick subscribe variables
          $personalizedHtml = $this->api->replaceOneclickSubscribeVars($personalizedHtml, $recipient);
          ...
          // Add open and click trackers
          $personalizedHtml = $this->addTrackers('cmnltrck', $newsletter->getId(), $memberId, $personalizedHtml);

          // Create a new mailing entry
          $this->api->addMailObject($mailingId, $memberId, array(
   ...
   * @param $html
   */
  protected function addTrackers($variable, $nlId, $rowId, $html)
  {
    if (!defined('LBWP_LOCALMAIL_NO_TRACKING')) {
      // In tracking mode, add an open tracker grafic and prefixes to track all external link clicks
      $base = 'https://' . LBWP_HOST . '/cms/plugins/lbwp/views/api/lm/';
      $img = '<img src="'.$base.'open.php?nl=' . $nlId . '&m=' . $rowId . '" alt="systemgrafik, zählt öffnungsrate"  />';
      $html = str_replace('{trackableUrl}', $base.'click.php?nl=' . $nlId . '&m=' . $rowId . '&url=', $html);
      $html = str_replace('<!--' . $variable . '-->', $img, $html);
    } else {
      // With no tracking we just dont add the open tracker and tracking prefix for urls
  ...
  {
    $info = array(
      'Beachten Sie, dass der lokale Versand erst freigegeben werden muss.',
  ...
  public function subscribe($data, $listId = '')
  {
    ...
    // Define the record id
    $recordId = md5($data['email']);

    // Send double optin mail instead of subscribing if the list needs it
    if ($this->api->needsDoubleOptin($listId)) {
      return $this->api->sendDoubleOptInMail($data['email'], array_merge($data, array(
        'recordId' => $recordId,
        'listId' => $listId
      )));
    }
