...
class Helper
{
  ...
  /**
   * @param int $eventId the event id
   * @param string $uid a contact uid
   * @return bool true, if the triggering was successful
   */
  public function triggerEventWithUid($eventId, $uid)
  {
    $endpoint = sprintf('event/%d/trigger', $eventId);
    $data = array(
      'key_id' => 'uid',
   ...
   * @return bool
   */
  public function updateOptIn($uid, $additional = array())
  {
    $data = array(
      'key_id' => 'uid',
      'uid' => $uid,
   ...
   * @return bool
   */
  public function updateField($id, $uid, $key, $value)
  {
    $data = array(
      'key_id' => $id,
      $id => $uid,
    ...
    $update = $this->request('contact', $data, 'PUT');
    SystemLog::add('Emarsys', 'debug', 'updateFieldResult', $update);
    if (isset($update['data']['id'])) {
      return true;
    }
   ...
   * @return bool
   */
  public function removeOptIn($uid, $additional = array())
  {
    $data = array(
      'key_id' => 'uid',
      'uid' => $uid,
      '31' => '2' // Set Opt-in Field to false
    );
   ...
   * @return array|mixed
   */
  public function removeFromList($campaignId, $uid, $listId)
  {
    return $this->request('email/unsubscribe', array(
      'email_id' => $campaignId,
      'contact_uid' => $uid,
      'launch_list_id' => $listId,
    ), 'POST');
  }
