...
class PageSettingsBackend
{
  ...
  {
    if (strlen($value) == 0) {
      $value = intval($config['defaultIfempty']);
    }
    $attr = array(
      'type' => 'number',
  ...
  public function displayFieldUpload($config, $value, $html)
  {
    ...
    if (is_string($url) && Strings::checkURL($url)) {
   ...
   * @return string the resulting html code
   */
  public function displayFieldTypedropdown($config, $value, $html)
  {
    // Create the dropdown field
    $value = ArrayManipulation::forceArray($value);
    $field = '<select id="' . $config['fieldId'] . '" multiple name="' . $config['fieldId'] . '[]">';
    $types = array_keys($config['typeMap']);
    ...
    $fieldsHtml = array(array(), array());
    foreach ($values as $post) {
      $selected = in_array($post->ID, $value) ? 'selected="selected"' : '';
      $theHtml =  '<option value="' . $post->ID . '" ' . $selected . '>' . $post->post_title . ' (' . $config['typeMap'][$post->post_type] . ')</option>';

      if($selected){
        $fieldsHtml[1][array_search($post->ID, $value)] = $theHtml;
      }else{
        array_push($fieldsHtml[0], $theHtml);
      }
    }

    ksort($fieldsHtml[1]);

    $field .= implode(PHP_EOL, $fieldsHtml[1]) . implode(PHP_EOL, $fieldsHtml[0]);
    $field .= '
      </select>
      <script type="text/javascript">
        jQuery(function() {
          jQuery("#' . $config['fieldId'] . '").chosen().chosenSortable();
        });
      </script>  
    ';
   ...
   * @param array $item the item to be saved
   */
  public function saveFieldTypedropdown($item)
  {
    $value = array_map('intval', $_POST[$item['id']]);
    update_option($item['id'], $value);
  }
