...
class WpMenu
{
  ...
  /**
   * Can build a tree of items from a sole list with parents
   * It's made simple and only supports 2 levels yet
   * @param \WP_Post[] $items
   * @return
   */
  public static function getTreeFromItems($items, $levels = 2, $cacheKey = '')
  {
    // Not from cache, build new
    $isHome = $_SERVER['REQUEST_URI'] == '/';
    $tree = array();
    foreach ($items as $item) {
      if ($item->menu_item_parent == 0) {
        $tree[$item->ID] = self::getMenuItemFromPost($item);
      }
    }
    ...
    if ($levels >= 2) {
      foreach ($items as $item) {
        if (!$isHome) {
          self::maybeFixCurrentItemClass($item);
        }
        if (isset($tree[$item->menu_item_parent])) {
          $tree[$item->menu_item_parent]['sub'][$item->ID] = self::getMenuItemFromPost($item);
        }
      }
    }
  ...
  public static function maybeFixCurrentItemClass(&$item)
  {
    if (str_ends_with($item->url, $_SERVER['REQUEST_URI'])) {
   ...
   * @return void
   */
  protected static function getMenuItemFromPost($item)
  {
    return array(
      'id' => $item->ID,
      'title' => $item->title,
      ...
      'target' => $item->target,
      'sub' => array(),
      'meta' => WordPress::getAccessiblePostMeta($item->ID, true)
    );
  }
} 