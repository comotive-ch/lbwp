...
class OpenIDConnectClient
{
  ...
  /**
   * @var string http proxy port if necessary
   */
  private $httpProxyPort;

  /**
   * @var string http proxy pwd if necessary
   */
  private $httpProxyPwd;
  ...
  public function __construct($provider_url = null, $client_id = null, $client_secret = null, $issuer = null) {
    require_once ABSPATH . '/wp-content/plugins/lbwp/resources/libraries/phpseclib1.0.2/autoload.php';
  ...
  public function requestUserInfo($attribute = null) {
    ...
    $content = $this->fetchURL($user_info_endpoint,null,$headers);
    $user_json = json_decode($content);
    if ($this->getResponseCode() <> 200) {
      SystemLog::add('OpenIDConnectClient::requestUserInfo', 'debug', $content, $_SESSION);
  ...
  protected function fetchURL($url, $post_body = null, $headers = array()) {
    ...
    if (isset($this->httpProxy)) {
      ...
      curl_setopt($ch, CURLOPT_PROXYPORT, $this->httpProxyPort);
      curl_setopt($ch, CURLOPT_PROXYUSERPWD, $this->httpProxyPwd);
    }
   ...
   */
  public function setHttpProxy($httpProxy, $port = '', $pwd = '') {
    $this->httpProxy = $httpProxy;
    $this->httpProxyPort = $port;
    $this->httpProxyPwd = $pwd;
  }
