...
class SimpleField
{
   ...
   * @return string html to represent the field
   */
  public static function displaySerializedObjectField($args, $key, $html, $fields = array())
  {
    // Get the current value
    list($option, $subkey) = explode('__', $args['key']);
    if (isset($_GET['page']) && str_starts_with($_GET['page'], 'wc-orders--')) {
      $order = wc_get_order($args['post']->ID);
      $arrayObject = $order->get_meta($option);
      $value = empty($arrayObject) ? '' : $arrayObject[$subkey];
    } else {
      $value = ArrayManipulation::forceArray(get_post_meta($args['post']->ID, $option, true))[$subkey];
    }
