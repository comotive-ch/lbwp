...
class PostTypeDropdown
{
   ...
   */
  public static function createDropdownArguments($types, $args, $key, $postId)
  {
    ...
    // Get currently selected items and add them (for when viewing a DE post in FR, that
    // has DE elements attached, but postItems only load FR items, so they go missing on save
    if (Multilang::isActive()) {
      $availableIds = array();
      foreach ($postItems as $postItem) $availableIds[] = $postItem->ID;
      $value = get_post_meta($postId, $key);
      if (is_array($value)) {
        $selectedIds = array_filter($value);
        // Add the selected ids to the post items array if not available
        foreach ($selectedIds as $selectedId) {
          if (!in_array($selectedId, $availableIds)) {
            $postItems[] = get_post($selectedId);
          }
        }
      }
    }

    // Extend title with language, if needed
    if (isset($args['show_language_tag']) && $args['show_language_tag']) {
      foreach ($postItems as $key => $item) {
        $postItems[$key]->post_title_alt = $postItems[$key]->post_title . ' (' . strtoupper(Multilang::getPostLang($item->ID)) . ')';
      }
    }
    ...
    // Allow to completely change the items list
    if (isset($args['itemOverallCallback']) && is_callable($args['itemOverallCallback'])) {
      $args['items'] = call_user_func($args['itemOverallCallback'], $args['items'], $postItems);
    }
  ...
  public static function addNewPostTypeItem()
  {
    ...
    $newPostId = intval(wp_insert_post(array(
      ...
      'post_type' => $postType,
      'post_status' => 'draft',
      'post_parent' => ($postType == 'onepager-item' || $postType == 'lbwp-selection-item') ? $assignedPostId : 0,
    )));
  ...
  public static function getPostElementName($post, $map = array(), $stati = array('draft' => 'Entwurf', 'pending' => 'Review', 'future' => 'Geplant'))
  {
    ...
    if (isset($post->post_title_alt) && strlen($post->post_title_alt) > 0) {
      $itemTitle = $post->post_title_alt;
    }
