...
class ChosenDropdown
{
  ...
  public static function addNewItemHtml($key, $args)
  {
    ...
    $sortBtn = '';
    if($args['auto_sorting']){
      $uniqueId = 'sort-dropdown-' . random_int(0, 99999);
      $sortBtn = '<div class="sort-dropdown-item">
        <a id="' . $uniqueId . '" class="button" href="#">Alphabetisch Sortieren</a>
      </div>
      <script>
        let sortBtn = jQuery("#' . $uniqueId . '");
        
        sortBtn.click(function(e){
          e.preventDefault();
          
          let select = sortBtn.parent().siblings("select");
  ...
  public static function getUserHtmlCallback($user)
  {
    ...
    return '
      <div class="mbh-chosen-inline-element">
        <h2><a href="' . $editLink . '" class="open-modal">' . apply_filters('mbh_userdropdown_display_name', $user->display_name, $user) . '</a></h2>
