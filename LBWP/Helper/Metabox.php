...
class Metabox
{
  ...
  protected function __construct($posttype)
  {
    ...
    switch ($posttype) {
      ...
      case 'shop_order':
        add_action('woocommerce_process_shop_order_meta', array($this, 'saveHposCompatMetabox'), 10, 2);
        add_action('add_meta_boxes', array($this, 'hposCompatMetaboxHandling'));
        break;
      default:
   ...
   * @return void
   */
  public function hposCompatMetaboxHandling()
  {
    $this->filterMetaboxes();
    $this->mergeSections();

    $screen = wc_get_container()->get(CustomOrdersTableController::class)->custom_orders_table_usage_is_enabled()
      ? wc_get_page_screen_id(str_replace('_', '-', $this->posttype))
      : $this->posttype;
   ...
   * @return void
   */
  public function saveHposCompatMetabox($orderId, $order)
  {
    // Only in HPOS mode
    ...
    foreach ($this->fields as $fields) {
      foreach ($fields as $field) {
        $postKey = $orderId . '_' . $field['key'];
        $value = $_POST[$postKey];
        if ($value === null) {
          continue;
        }
        // Handle meta fields differently when __ is present
        if (str_contains($field['key'], '__')) {
          list($metaKey, $innerKey) = explode('__', $field['key']);
          $current = $order->get_meta($metaKey, true);
          if (empty($current)) {
            $current = array();
          }
          $current[$innerKey] = $value;
          $order->update_meta_data($metaKey, $current);
        } else {
          $order->update_meta_data($field['key'], $value);
        }
      }
    }
  ...
  public function saveMetabox($postId)
  {
    ...
    // If different post type, skip
    if (get_post($postId)->post_type != $_REQUEST['post_type']) {
      return;
    }
  ...
  protected function enqueueAssets()
  {
    ...
    if (Strings::startsWith($_SERVER['SCRIPT_NAME'], '/wp-admin/post') || str_starts_with($_GET['page'], 'wc-orders')) {
      wp_enqueue_script(
   ...
   * @param $boxId
   */
  public function addGroupStart($groupId, $boxId, $itemId) {
    $args['html'] = '
      <div class="mbh-field-grouper start" data-id="' . $groupId . '" data-item="' . $itemId . '">
        <a href="javascript:void(0)" class="mbh-move-group-up">Nach oben</a> | 
        <a href="javascript:void(0)" class="mbh-move-group-down">Nach unten</a> 
        <input type="hidden" name="mbh-group[]" value="' . $groupId . '" />
      </div>     
    ';
    $this->addField('group-' . $groupId, $boxId, $args, array($this, 'displayHtml'), array($this, 'saveGroup'));
  }
   ...
   * @param $boxId
   */
  public function saveGroup($postId, $field, $boxId)
  {
    update_post_meta($postId, 'mbh-group-list', $_POST['mbh-group']);
  }
   ...
   * @return array
   */
  public static function getGroupIdList($postId, $prefix, $fallback)
  {
    $idList = array();
    $raw = get_post_meta($postId, 'mbh-group-list', true);
    if (is_array($raw)) {
      foreach ($raw as $item) {
        $idList[] = intval(str_replace($prefix, '', $item));
      }
      // Add new empty slides, if not matching fallback count
      if (count($idList) < $fallback) {
        for ($i = $fallback; count($idList) < $fallback;$i--) {
          $idList[] = $i;
        }
      }
    } else {
   ...
   * @param $boxId
   */
  public function addGroupEnd($groupId, $boxId, $itemId)
  {
    $this->addHtml('group-end-' . $groupId, $boxId, '
      <div class="mbh-field-grouper end" data-id="' . $groupId . '" data-item="' . $itemId . '"></div>
    ');
  }
   ...
   * @param array $args additional arguments: description, width
   */
  public function addSerializedObjectField($key, $boxId, $title, $args = array())
  {
    // Add the title as an argument
    $args['title'] = $title;
    ...
    $this->addField(
      $key, $boxId, $args,
      array($this, 'displaySerializedObjectField'),
      array($this, 'saveSerializedObjectField')
    );
  }
   ...
   * @param array $args additional arguments: description, width
   */
  public function addEditableTable($key, $boxId, $args = array())
  {
    // Add the field
    $this->addField(
      $key, $boxId, $args,
      array($this, 'displayEditableTable'),
      array($this, 'saveEditableTable')
    );
  }
  ...
  public function addUserDropdown($key, $boxId, $title, $args = array())
  {
    ...
    foreach ($users as $user) {
      $items[$user->ID] = array(
        'title' => apply_filters('mbh_userdropdown_display_name', $user->display_name, $user),
        'data' => array(
  ...
  {
    // This is sometimes called at admin_init, but only needs to work on edit screens
    if (
  ...
  /**
   * @param array $args displays an editable table
   */
  public function displayEditableTable($args)
  {
    $key = $args['post']->ID . '_' . $args['key'];
    $html = $this->getTemplate($args, $key, 'empty_desc');
    $data = get_post_meta($args['post']->ID, $args['key'], true);
    $data = ArrayManipulation::forceArray($data);

    $table = '<table class="mbh-editable-table" data-key="' . $key . '" data-last-id="' . intval(array_key_last($data)) . '">';
    // Build the headings
    $table .= '<tr class="heading">';
    // Remove the id column as it's not needed / automatically generated
    foreach ($args['columns'] as $value) {
      $table .= '<td>' . $value . '</td>';
    }
    if (isset($args['allow_delete']) && $args['allow_delete']) {
      $table .= '<td class="icon"><span class="dashicons dashicons-trash deletion-allowed"></span></td>';
    }
    $table .= '</tr>';

    // Build the actual table content
    foreach ($data as $id => $row) {
      $table .= '<tr data-id="' . intval($id) . '">';
      foreach ($args['columns'] as $column => $value) {
        $table .= '
          <td>
            <input type="text" name="' . $key . '[' . intval($id) . '][' . $column . ']" value="' . esc_attr($row[$column]) . '" />
          </td>
        ';
      }
      // Handle deletion of rows
      if (isset($args['allow_delete']) && $args['allow_delete']) {
        $table .= '<td class="icon"><a href="javascript:void(0)" class="mbh-editable-table-delete dashicons dashicons-trash"></a></td>';
      }
      $table .= '</tr>';
    }
    $table .= '</table>';
    // Add new data set if allowed
    if (isset($args['allow_add']) && $args['allow_add']) {
      $table .= '
        <p>
          <a href="javascript:void(0)" class="mbh-editable-table-add button" data-columns="' . esc_attr(json_encode($args['columns'])) . '">
          <span class="dashicons dashicons-plus"></span>Neuen Datensatz hinzufügen</a>
        </p>
      ';
    }
   ...
   * @return string HTML code to display the field
   */
  public function displaySerializedObjectField($args)
  {
    $key = $args['post']->ID . '_' . $args['key'];
    $template = $this->getTemplate($args, $key);
    return SimpleField::displaySerializedObjectField($args, $key, $template);
  }
  ...
  public function displayDateField($args, $addTimeField = false, $useIntegerConversion = false)
  {
    ...
    $dataDefaultDateFrom = '';
    if (isset($args['default_date_from']) && strlen($args['default_date_from']) > 0) {
      $dataDefaultDateFrom = ' data-default-date-from="' . $args['post']->ID . '_' . $args['default_date_from'] . '" ';
    }
    ...
    $input = '
      <input type="text" id="' . $key . '" name="' . $key . '" class="mbh-datefield" ' . $dataDefaultDateFrom . ' value="' . esc_attr($value) . '"' . $attr . ' />
      ' . $timeField . '
    ';
   ...
   * @param $boxId
   */
  public function saveSerializedObjectField($postId, $field, $boxId)
  {
    $value = $_POST[$postId . '_' . $field['key']];
    list($option, $subkey) = explode('__', $field['key']);
    $current = ArrayManipulation::forceArray(get_post_meta($postId, $option, true));
    $current[$subkey] = $value;
    update_post_meta($postId, $option, $current);
  }
   ...
   * @param string $boxId the metabox id
   */
  public function saveEditableTable($postId, $field, $boxId)
  {
    // Validate and save the field
    $data = $_POST[$postId . '_' . $field['key']];
    // Save this for now with almost no validation at all
    update_post_meta($postId, $field['key'], $data);
  }
  ...
  public function saveDateTimeField($postId, $field, $boxId)
  {
    ...
    if (
      ...
      // No saving, since not valid, but explicit deletion of option
